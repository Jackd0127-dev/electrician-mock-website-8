...
const projects: Project[] = [
  { title: 'Riverside Mews', summary: 'A carefully coordinated rewire and lighting upgrade across a period residential development.', location: 'Demo · Belfast', category: 'Residential upgrade', image: '/images/fabian-kleiser-CW0T7Rce2SA-unsplash.jpg' },
  { title: 'Beacon Yard', summary: 'Power distribution, LED lighting and EV-ready infrastructure for a mixed-use workspace.', location: 'Demo · Lisburn', category: 'Commercial fit-out', image: '/images/641c1cd1810de2e16b69977cc41358bc.jpg' },
  { title: 'Hillside Workshop', summary: 'A dependable three-phase supply upgrade, containment and task lighting installation.', location: 'Demo · Newry', category: 'Industrial installation', image: '/images/mark-kats-oj1zW_PNI4k-unsplash.jpg' },
  { title: 'Oakfield Homes', summary: 'Domestic electrical first-fix, smart lighting and compliant final certification.', location: 'Demo · Bangor', category: 'New-build residential', image: '/images/nastuh-abootalebi-yWwob8kwOCk-unsplash.jpg' },
  { title: 'North Coast Charging', summary: 'A scalable workplace EV charging scheme designed around future demand.', location: 'Demo · Coleraine', category: 'EV infrastructure', image: '/images/zaptec-UHNdOFqNhNQ-unsplash.jpg' },
  { title: 'Harbour House', summary: 'A refined apartment renovation with concealed services and considered feature lighting.', location: 'Demo · Holywood', category: 'Residential renovation', image: '/images/e32375219bc4107fccd81055667006e8.jpg' },
]
...
function LinkButton({ href, children, variant = 'primary' }: { href: string; children: React.ReactNode; variant?: 'primary' | 'light' | 'text' }) {
  return <a className={`button button--${variant}`} href={href}>{children}{variant !== 'text' && <ArrowRight size={17} strokeWidth={2.4} />}</a>
}

function Logo({ light = false }: { light?: boolean }) {
  return <img className="brand-logo" src={`/brand/JD-Electrical_Primary-Horizontal_${light ? 'White' : 'Colour'}.svg`} alt="JD Electrical" />
}
...
function Header({ page }: { page: string }) {
  ...
  return <header className="site-header">
    <a href="/" className="logo-link" aria-label="JD Electrical home"><Logo light /></a>
    <nav className="desktop-nav" aria-label="Main navigation">
      {nav.map((item, index) => <span className="nav-item" key={item.href}><a className={page === item.href ? 'active' : ''} href={item.href}>{item.label}</a>{index < nav.length - 1 && <ChevronRight size={13} />}</span>)}
    </nav>
    <a className="header-cta" href="/contact">Send an enquiry</a>
    ...
    {open && <div className="mobile-menu">
      <a href="/" className="menu-logo"><Logo light /></a>
      <button className="menu-close" type="button" aria-label="Close menu" onClick={() => setOpen(false)}><X /></button>
      <nav aria-label="Mobile navigation">{[...nav, { label: 'Contact', href: '/contact' }].map(item => <a href={item.href} key={item.href}>{item.label}<ArrowUpRight size={18} /></a>)}</nav>
      <a className="button button--light" href="/contact">Start a project<ArrowRight size={17} /></a>
    </div>}
  </header>
}
...
function Footer() {
  return <footer className="site-footer">
    <div className="footer-main">
      ...
      <div><span className="footer-kicker">Quick links</span><a href="/">Home</a><a href="/about">About</a><a href="/projects">Projects</a><a href="/services">Services</a><a href="/contact">Contact</a></div>
...
function ServiceList({ detailed = false }: { detailed?: boolean }) {
  return <div className={detailed ? 'services-list services-list--detailed' : 'services-list'}>{services.map((service, index) => {
    ...
    return <article className="service-card" key={service.title}><span className="service-number">0{index + 1}</span><Icon className="service-icon" size={30} strokeWidth={1.7} /><h3>{service.title}</h3><p>{service.text}</p>{detailed && <ul>{service.bullets.map(bullet => <li key={bullet}><CheckCircle2 size={15} />{bullet}</li>)}</ul>}<a href="/contact" aria-label={`Enquire about ${service.title}`}><ArrowUpRight size={21} /></a></article>
  })}</div>
}
...
function Home() {
  return <>
    ...
    <section className="intro-section"><div className="intro-copy"><p className="eyebrow">Who we are</p><h2>Reliable electrical work with a sharper standard of care.</h2><p>JD Electrical brings calm, considered delivery to every domestic and commercial job. We believe great work is as much about communication and respect as it is about a technically precise finish.</p><div className="hero-actions"><LinkButton href="/about">Our approach</LinkButton><LinkButton href="/projects" variant="text">See our work</LinkButton></div></div><div className="proof-panel" style={{ backgroundImage: "linear-gradient(135deg, rgba(0,24,46,.86), rgba(0,24,46,.44)), url('/images/mark-kats-oj1zW_PNI4k-unsplash.jpg')" }}>{proofPoints.map(point => { const Icon = point.icon; return <div className="proof-card" key={point.value}><Icon size={21} /><strong>{point.value}</strong><span>{point.label}</span></div> })}</div></section>
    <section className="featured-project"><div className="featured-image" style={{ backgroundImage: "url('/images/nastuh-abootalebi-yWwob8kwOCk-unsplash.jpg')" }} /><div className="featured-copy"><p className="eyebrow">Featured demo project</p><h2>Oakfield Homes</h2><p>A polished new-build electrical package, from practical first-fix work through to warm, energy-efficient lighting and final certification.</p><LinkButton href="/projects" variant="text">View all projects</LinkButton><div className="featured-stats"><div><strong>5</strong><span>homes in this demo phase</span></div><div><strong>3</strong><span>service areas coordinated</span></div></div></div></section>
